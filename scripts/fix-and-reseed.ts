...
async function fixAndReseed() {
    console.log('🧹 1. Revirtiendo partido a status "completed"...');
    await db.doc(`matches/${MATCH_ID}`).update({ status: 'completed' });

    console.log('🧹 2. Borrando evaluaciones residuales y duplicadas...');
    const evalsSnap = await db.collection('evaluations').where('matchId', '==', MATCH_ID).get();
    for (const doc of evalsSnap.docs) await doc.ref.delete();

    const selfEvalsSnap = await db.collection(`matches/${MATCH_ID}/selfEvaluations`).get();
    for (const doc of selfEvalsSnap.docs) await doc.ref.delete();

    const processedSnap = await db.collection(`matches/${MATCH_ID}/processedSubmissions`).get();
    for (const doc of processedSnap.docs) await doc.ref.delete();

    const subsSnap = await db.collection('evaluationSubmissions').where('matchId', '==', MATCH_ID).get();
    for (const doc of subsSnap.docs) await doc.ref.delete();

    console.log('🧹 3. Restaurando assignments a "pending"...');
    const asgSnap = await db.collection(`matches/${MATCH_ID}/assignments`).get();
    for (const doc of asgSnap.docs) {
        await doc.ref.update({ status: 'pending', evaluationId: null, autoCompleted: false });
    }

    console.log('🧹 4. Reseteando Stats Históricas de los 10 jugadores a 0 (para arreglar el MVPx11) y OVR 50...');
    const matchSnap = await db.doc(`matches/${MATCH_ID}`).get();
    const playersIds = matchSnap.data()?.playerUids || [];
    for (const pid of playersIds) {
        await db.doc(`players/${pid}`).update({
            ovr: 50,
            pac: 50, sho: 50, pas: 50, dri: 50, def: 50, phy: 50,
            stats: { mvpVotes: 0, matchesPlayed: 0, goals: 0, assists: 0, averageRating: 0 }
        });
    }

    console.log('🌱 5. Sembrando 10 submissions limpias y perfectas...');
    // --- EXACT SEEDING LOGIC FROM BEFORE ---
    const idMap: Record<string, string> = {
        'Alvaro M': '3xcfOetChgYB4ax6oh3w5zzd2w82', 'José P': '4Hjt7RpO28Trff70J2Z6YMSJshx1',
        'Eminencia': 'EFKPK6vDW4WrEosD6UrFRDGMG543', 'Fiorenzo': 'N7GKXwOa4vVeap4emu23x8jn8wF2',
        'Doroteo': 'QYx3MCcrYRTJ1aYB24tQy0A2fmM2', 'Aurelio': '7LRVMYdOI8Sm9gahfBkh5rbVENt1',
        'Briseida': 'JAn8pNSZoNP7rWWyZJD1gCFBC5t2', 'Liroy': 'dRYXgsJ1Joa28L69MV9kFRpWfxC3',
        'Santiago López': 'njH0OBBrmzSWOeoXamHUZyxavLv1', 'Cosme': 'qrVOXyawztZBtTg7i6md6Opbt5H2'
    };
    ...
    const assignments = asgSnap.docs.map(d => ({ id: d.id, ...d.data() } as Assignment));
    let submissionsCount = 0;

    for (const uid of playersIds) {
        const pSnap = await db.doc(`players/${uid}`).get();
        const p = { id: pSnap.id, ...pSnap.data() } as any;

        const myStats = playerStats[uid] || { g: 0, a: 0, chronicle: 'Buen partido.' };
        const myAssignments = assignments.filter(a => a.evaluatorId === uid);
        const misEvaluaciones: any[] = [];

        for (const assignment of myAssignments) {
            const sujetoSnap = await db.doc(`players/${assignment.subjectId}`).get();
            const sujeto = { id: sujetoSnap.id, ...sujetoSnap.data() } as any;

            const cacheKey = `${uid}_${assignment.subjectId}`;
            const plan = explicitEvals[cacheKey] || createDynamicEval(assignment.subjectId, uid);

            if (plan.type === 'points') {
                misEvaluaciones.push({ assignmentId: assignment.id, subjectId: sujeto.id, displayName: sujeto.name, photoURL: sujeto.photoURL || '', position: sujeto.position, evaluationType: 'points', rating: plan.rating });
            } else if (plan.type === 'tags') {
                misEvaluaciones.push({ assignmentId: assignment.id, subjectId: sujeto.id, displayName: sujeto.name, photoURL: sujeto.photoURL || '', position: sujeto.position, evaluationType: 'tags', performanceTags: plan.tags });
            } else if (plan.type === 'text') {
                misEvaluaciones.push({ assignmentId: assignment.id, subjectId: sujeto.id, displayName: sujeto.name, photoURL: sujeto.photoURL || '', position: sujeto.position, evaluationType: 'text', textDescription: plan.text, aiAnalysisComplete: true, aiConfidence: plan.ai.confidence, aiSummary: plan.ai.summary, aiAttributeChanges: plan.ai.changes });
            }
        }

        const submissionData = {
            evaluatorId: uid, matchId: MATCH_ID, submittedAt: new Date().toISOString(),
            submission: { evaluatorGoals: myStats.g, evaluatorAssists: myStats.a, personalChronicle: myStats.chronicle, mvpVote: MVP_ID, evaluations: misEvaluaciones }
        };
    ...
    console.log(`🎉 ¡Plaqueta lista! ${submissionsCount} submissions inyectadas. Entrar en panel y Finalizar en aprox 20 seg.`);
}

fixAndReseed().catch(e => { console.error(e); process.exit(1); }).finally(() => process.exit(0));
