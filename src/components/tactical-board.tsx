...
import type { Match, Team, Player as PlayerType, Formation, TeamFormation } from '@/lib/types';
...
import { Loader2, Save, MoreVertical, RefreshCw } from 'lucide-react';
...
const TacticalPlayer = ({ player, onOpenMenu }: { player: PlayerType, onOpenMenu: (player: PlayerType) => void }) => (
    <DropdownMenu>
        <DropdownMenuTrigger asChild>
            <button className="flex flex-col items-center gap-1 group text-center w-20 cursor-pointer">
        ...
        <DropdownMenuContent onClick={(e) => e.stopPropagation()}>
           <DropdownMenuItem onClick={() => onOpenMenu(player)}>
               Mover Jugador
           </DropdownMenuItem>
        </DropdownMenuContent>
    </DropdownMenu>
...
export function TacticalBoard({ match: initialMatch }: TacticalBoardProps) {
  ...
  const [movingPlayer, setMovingPlayer] = useState<PlayerType | null>(null);
  ...
  const handleMovePlayer = (playerToMoveId: string, toTeamIndex: number | 'bench') => {
    ...
    if (fromTeamIndex === -1 || !playerInfo) {
        setMovingPlayer(null);
        return;
    }
    
    // Remove from source team's player list
    newTeams[fromTeamIndex].players = newTeams[fromTeamIndex].players.filter(p => p.uid !== playerToMoveId);

    // Add to target team's player list if it's a team
    if (toTeamIndex !== 'bench' && toTeamIndex > -1) {
    ...
    // Re-assign formations and calculate OVR for affected teams
    ...
    if (formation) {
      newTeams.forEach((team, index) => {
          if (index === fromTeamIndex || index === toTeamIndex) {
              const teamPlayers = team.players.map(p => allPlayersMap.get(p.uid)).filter(p => p) as PlayerType[];
  ...
  const handleOpenMenu = (player: PlayerType) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Prune circular references or large objects before saving
      const teamsToSave = teams.map(team => ({
          ...team,
          suggestedFormation: selectedFormationName,
          // Ensure we only save the IDs, not the full player objects
  ...
  const allAssignedPlayerIds = new Set(teams.flatMap(t => Object.values(t.formation || {})));
  const benchPlayers = Array.from(allPlayersMap.values()).filter(p => !allAssignedPlayerIds.has(p.id));
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
            <div className="xl:col-span-10">
                ...
                {teams.map((team, teamIndex) => (
                    <Card key={team.name}>
                        ...
                        <CardContent>
                            <div className="relative aspect-[7/10] bg-green-700/20 dark:bg-green-900/30 rounded-lg overflow-hidden flex items-center justify-center">
            ...
            <div className="xl:col-span-2">
                <Card>
                    <CardHeader>
                        <CardTitle>Suplentes ({benchPlayers.length})</CardTitle>
                    </CardHeader>
        ...
        {movingPlayer && (
            <DropdownMenu open={!!movingPlayer} onOpenChange={() => setMovingPlayer(null)}>
                <DropdownMenuTrigger />
                <DropdownMenuContent>
                    {teams.map((team, index) => (
                        <DropdownMenuItem key={team.name} onClick={() => handleMovePlayer(movingPlayer.id, index)}>
                            Mover a {team.name}
                        </DropdownMenuItem>
                    ))}
                </DropdownMenuContent>
