...
import { mainNavItems, extraNavItems } from './nav-config';
                ...
                <div className="flex h-14 w-full items-center justify-around font-medium">
                    ...
                    <div className="relative flex items-center justify-center">
                        <button
                            ...
                            className={cn(
                                'group relative inline-flex flex-col items-center justify-center gap-1 px-3 py-1 rounded-xl transition-all duration-200 active:scale-95',
                                isMatchesActive ? 'text-primary' : 'text-muted-foreground hover:text-foreground'
                            )}
                            aria-haspopup="true"
                            aria-expanded={matchesMenuOpen}
                        >
                            <Calendar className={cn('h-5 w-5 transition-all duration-200', isMatchesActive && 'stroke-[2.5px]')} />
                            <span className="text-[10px] leading-none font-medium">Partidos</span>
                            {/* Dot indicator */}
                            {isMatchesActive && (
                                <motion.span
...
function MobileNavItem({ item, pathname }: { item: any, pathname: string }) {
    ...
    return (
        ...
        >
            ...
            <span className="relative text-[10px] leading-none font-medium">{item.label}</span>
            {/* Animated dot indicator — slides between tabs */}
            {isActive && (
                <motion.span
                    layoutId="mobile-nav-indicator"
                    className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 h-1 w-1 rounded-full bg-primary"
...
const BottomSheetMenu = React.forwardRef<HTMLDivElement, { pathname: string, onClose: () => void }>(({ pathname, onClose }, ref) => (
    <motion.div
        ...
        className="fixed inset-x-0 bottom-0 z-[60] cursor-grab active:cursor-grabbing"
    >
        <div
            ref={ref}
